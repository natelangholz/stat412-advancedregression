...
#there is also a different library for genetic algorithms (library(GA)) which seems more multi-purpose

library(genalg)
library(ggplot2)

dataset <- data.frame(item = c("pocketknife", "beans", "potatoes", "onions", "sleeping bag", "rope", "compass"), survivalpoints = c(10, 20, 15, 2, 30, 10, 30), weight = c(1, 5, 10, 1, 7, 5, 1))
weightlimit <- 20
...
iter = 100
GAmodel <- rbga.bin(size = 7, popSize = 100, iters = iter, mutationChance = 0.01, 
