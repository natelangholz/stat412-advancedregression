...
paths <- dir("week-9/data/holmes_stories", full.names = TRUE)
...
stopwords <- readLines("week-9/data/stopwords_en.txt")
...
nlp <- read_csv("week-9/data/sh_nlp.csv.gz")

nlp

sh_nlp <- read_csv("week-9/data/sh_nlp.csv.gz")
...
## ------------------------------------------------------------------------
sh_nlp %>% filter(tolower(word) != lemma) %>% dplyr::select(word, lemma) %>% print(n = 10)
...
## ------------------------------------------------------------------------
sh_compound %>% dplyr::select(id, thing) %>% print(n = 10)

## ------------------------------------------------------------------------
results <- sh_nlp %>%
  dplyr::select(id, entity, entity_type) %>%
  filter(!is.na(entity))
...
sh_nlp %>% filter(id == 1, sid == 1) %>%
  dplyr::select(word, source, relation, word_source)
...
sotu_nlp <- read_csv("week-9/data/sotu.csv.gz")
sotu_meta <- read_csv("week-9/data/sotu_meta.csv")
...
summary_2001 <- sotu_nlp %>%
  ...
  dplyr::select(id, word, word_source) %$%
...
dplyr::select(sotu_pca, president, party, PC1, PC2)
...
sotu_tf <- sotu_nlp %>%
  ...
  get_tfidf(min_df = 0.05, max_df = 0.95,
            type = "tf", tf_weight = "raw")
tm <- read_rds("week-9/data/sotu_tm.Rds")
...
m2$pred <- predict(model, newx = mat$tf, type = "response",
                   s = model$lambda.1se)
dplyr::select(m2, new_id, id, sid, president, year, pred)
